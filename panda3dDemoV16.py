...
class MyApp(ShowBase):
    ...
    def changePlanetColor(self,num):
        ...
        self.planet.setTexture(self.planetTextures[mod][0])
        for land in self.landPieces:
            self.landPieces[land].setColor(*self.planetTextures[mod][1])
    ...
    def drawHomeScreen(self):
        ...
        homeTitleObject = OnscreenText(parent=self.helpFrame, text = homeTitle, pos = (1,-.35,0),  #set parent??
                scale = 0.3,fg=(1,0.5,0.5,1),align=TextNode.ACenter)
    ...
    def drawTextMain(self):  
        ...
        self.startYearObject= OnscreenText(text=startYearText, pos=(-.88,.63), scale=.07, fg=(1,.5,.5,1), align=TextNode.ACenter, mayChange=1)
        pText='13.8 billion years'
        self.endYearObject= OnscreenText(text=pText, pos=(0.7,.63), scale=.07, fg=(1,.5,.5,1), align=TextNode.ACenter, mayChange=1)
        currGal = "Galaxy %d" % (self.currMiddleModel)
        self.currGalObject= OnscreenText(text=currGal, pos=(1.1,-.50), scale=.07, fg=(1,.5,.5,1), align=TextNode.ACenter, mayChange=1)
    ...
    def drawHelpMenu(self):   
        ...
        helpText3 = "To increase glow, use 'h', and use 'g' to decrease glow"
        ...
        help2Obj = OnscreenText(parent=self.helpFrame, text = helpText2, pos = (1,-.8,0),  #set parent??
                scale = 0.07,fg=(1,0.5,0.5,1),align=TextNode.ACenter)
        help3Obj = OnscreenText(parent=self.helpFrame, text = helpText3, pos = (1,-1,0),  #set parent??
                scale = 0.07,fg=(1,0.5,0.5,1),align=TextNode.ACenter)
    ...
    def drawGalaxyMenu(self): #LOAD FRAME IN INIT + help frame!!!!!!!!!!!!!
        ...
        currGalObj = OnscreenText(parent=self.galFrame, text = currGal, pos = (1,-1,0),  #set parent??
                scale = 0.07,fg=(1,0.5,0.5,1),align=TextNode.ACenter)
        galaxyListTens = [str(i) for i in range(self.counter/10)] # am i missing one
        galaxyListOnes = [str(i) for i in range(9)]
        self.galaxyTens, self.galaxyOnes = str(0),str(0)
        self.galaxySelectorTens = DirectOptionMenu(parent=self.galFrame,text="options", scale=0.1,pos=(.8,0,-1.5),items=galaxyListTens,
                initialitem=0,highlightColor=(0.65,0.65,0.65,1),command=self.itemSelTens,textMayChange=1)
        self.galaxySelectorOnes = DirectOptionMenu(parent=self.galFrame,text="options", scale=0.1,pos=(1,0,-1.5),items=galaxyListOnes,
                initialitem=0,highlightColor=(0.65,0.65,0.65,1),command=self.itemSelOnes,textMayChange=1)
        self.goButton = DirectButton(parent=self.galFrame,pos=(1,0,-1.8), text="GO",
                                   scale=.1, pad=(.1, .1),
                                    command=self.closeGalaxyMenu)
    def itemSelOnes(self,arg):#NOT WORKING!!!
    ...
    def loadPlanetTextures(self):
        ...
        tex = True
        rgbPlanets = [(209/255.0,49/255.0,27/255.0),(31/255.0,190/255.0,41/255.0), (182/255.0, 218/255.0, 240/255.0), (81/255.0,167/255.0,89/255.0)]
