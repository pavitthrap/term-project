...
class MyApp(ShowBase):
    ...
    def closeGalaxyMenu(self):
        if int(self.newMiddleGal) != self.currMiddleModel: #then we need to switch gal
            try:
                ...
                self.currGalObject['text']= "Galaxy %d" % self.currMiddleModel
    ...
    def drawHomeScreen(self):
        ...
        imageObject = OnscreenImage(parent = self.helpFrame, image = 'textures/starBackground.jpg', 
            pos = (1, 0, -.8), hpr = (0,0,45), scale = 1.3)
        homeTitle = "Galaxy\nGlider"
    ...
    def initTutorialMessages(self):
        ...
        '''Use the time slider at the top to view the universe at different ages. Use the arrow keys, 'r' and 'l' to rotate your view. Click on the planet to explore it, and move around with arrow keys. Use 'g' to reduce the glow, and 'h' to increase it. ''')
    ...
    def drawTextMain(self):  
        ...
        self.startYearObject= OnscreenText(text=startYearText, pos=(-.88,.68), scale=.07, fg=(247/255.0, 218/255.0, 25/255.0,1), 
            align=TextNode.ACenter, mayChange=1)
        pText='13.8 billion years'
        self.endYearObject= OnscreenText(text=pText, pos=(0.7,.68), scale=.07, fg = (247/255.0, 218/255.0, 25/255.0,1), 
            align=TextNode.ACenter, mayChange=1)
        currGal = "Galaxy %d" % (self.currMiddleModel)
        self.currGalObject= OnscreenText(text=currGal, pos=(.95,-.45), scale=.07, fg=(247/255.0, 218/255.0, 25/255.0,1), 
            align=TextNode.ACenter, mayChange=1)
    ...
    def drawHelpMenu(self):   
        ...
        \nTo rotate, use 'r' for clockwise and 'l' for counterclockwise.
        \nTo increase glow, use 'h', and use 'g' to decrease glow.
        \nCLick on the planet to explore it.
