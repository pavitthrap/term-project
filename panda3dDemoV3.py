...
class MyApp(ShowBase):
    def __init__(self):
        ...
        #set detection - need collision sphere??
        #self.cs = CollisionSphere(0,1.8,0,10)
        self.planet.setTag('myObjectTag', '1')
        self.planetHover = False
        #self.csCol = CollisionNode('planetSphere')
        #self.pickerNode.addSolid(self.cs)
    ...
    def changeColorTask(self,task):
        ...
        self.midGalaxy.setColor(1.0*currR/256, 1.0*currG/256, 1.0*currB/255,1) #consider setColorScale
    ...
    def defineIntervals(self):
    	self.midGalaxyTurnInterval= Sequence(
    		self.midGalaxy.hprInterval(.3, (0,50,0)), #try changing this stuff
    		self.midGalaxy.hprInterval(2,(20,-30,0))
    		)
    ...
    def closeGalaxyMenu(self):
        ...
        if self.newMiddleGal != 'Current': #then we need to switch gal
            ...
            switch = self.galaxies[num].getPos()
            middle = self.midGalaxy.getPos()
            self.galaxies[num].setPos(middle)
            self.midGalaxy.setPos(middle)
            #self.midGalaxy, self.galaxies[num]
            self.currMiddleGal = num
    ...
    def drawGalaxyMenu(self): #LOAD FRAME IN INIT + help frame!!!!!!!!!!!!!
        ...
        galaxySelector = DirectOptionMenu(parent=self.galFrame,text="options", scale=0.1,items=galaxyList,
                initialitem=0,highlightColor=(0.65,0.65,0.65,1),command=self.itemSel,textMayChange=1, pos=(.8,0,-1.5))
    ...
    def loadMiddleModel(self,num):
    	self.midGalaxy = loader.loadModel('galaxies/gal%d' %(num))
    	self.midGalaxy.reparentTo(self.render) 
    	self.midGalaxy.setPos (0,0,0)
    	self.midGalaxy.setColor(*self.startColor)
    	#set texture 
        self.midGalaxy.setTexture(self.starTexture)
    ...
    def loadPlanets(self): # place planet in curr galaxy - which would actually just be the thing in center
        try: 
            ...
            #load texture?
            #self.planet.setTexture(self.starTexture)
