...
class MyApp(ShowBase):
    ...
    def loadTerrain(self):
        ...
        for xPos in range(-(2*self.blenderPlaneDim),2*self.blenderPlaneDim+1,self.blenderPlaneDim):
            for yPos in range (-self.blenderPlaneDim,self.blenderPlaneDim*3+1,self.blenderPlaneDim):
                ...
                self.land[self.landCounter].setPos(0, 0, 0)
                ...
                if num==0:
                    ...
                    center = self.getRandPos(xPos,yPos,zPos,interval) #may want to change range

                    #get number of trees
                    numTrees = random.randint(4,10)
                    self.makeTrees(numTrees,center,xPos,yPos,zPos)
    ...
    def makeTrees(self,numTrees, center, xPos,yPos,zPos):
        ...
        for position in positions:
            self.loadTree(*position)

    def loadTree(self,x,y,z):
        #change!!!!!! tree1 -> other possible trees - string formatting
        tree = loader.loadModel('modelsMisc/tree1')
