...
class MyApp(ShowBase):
    ...
    def closeGalaxyMenu(self):
        if int(self.newMiddleGal) != self.currMiddleModel: #then we need to switch gal
            try:
                ...
                self.currGalObject['text']= "Current: Galaxy %d" % self.currMiddleModel
    ...
    def loadBackground(self):
        ...
        #plight2.setColor((2,2,2,1))
        #(209/255.0,49/255.0,27/255.0)
        self.plight2.setColor(newColor)
    ...
    def initTutorialMessages(self):
        ...
        '''Use the time slider at the top to view the universe at different ages. Use the arrow keys, 'r' and 'l' to rotate your view. Click on the planet to explore it, and move around with arrow keys. Use 'g' to reduce the glow, and 'h' to increase it. Change the mass of the planet to change the gravitational redshift.''')
    ...
    def drawTextMain(self):  
        ...
        self.currGalObject= OnscreenText(text=currGal, pos=(1.45,-.24), scale=.07, fg=(1,1,1,1), 
            align=TextNode.ACenter, mayChange=1)
    ...
    def drawGravityMenu(self):   
        ...
        \nHit enter to submit value.
        \nUnits are in solar masses.
        \nPlease enter a float from 0 to 4.'''
        help1Obj = OnscreenText(parent=self.gravFrame, text = gravText, pos = (1,-.7,0),  #set parent??
                scale = 0.07,fg=(1,1,1,1),align=TextNode.ACenter)
        currentMass = "Current mass: %d solar masses" % (self.currMass)
    ...
    def drawGalaxyMenu(self): 
        ...
        currGalObj = OnscreenText(parent=self.galFrame, text = currGal, pos = (1,-.8,0),  #set parent??
                scale = 0.07,fg=(1,1,1,1),align=TextNode.ACenter)
        self.galaxyListTens = [str(i) for i in range(self.counter/10 + 1)] # am i missing one
        ...
        for xPos in range(-(3*self.blenderPlaneDim),3*self.blenderPlaneDim+1,self.blenderPlaneDim):
            for yPos in range (-self.blenderPlaneDim,self.blenderPlaneDim*3+1,self.blenderPlaneDim):
                ...
                if num==0:
                    ...
                    if num == 0 and self.spoutCounter<2: 
                        print('peep')
                        self.spouts[self.spoutCounter] = loader.loadModel('modelsMisc/pool')
                        self.spouts[self.spoutCounter].reparentTo(self.land[self.landCounter])
                        realScale = self.selectFloat(0.2,1)
                        center = self.getRandPos(0,0,0,interval-1)
                        self.spouts[self.spoutCounter].setPos(center)
                        self.spouts[self.spoutCounter].setColor(66/255.0,101/255.0,244/255.0,1)
                        self.spouts[self.spoutCounter].setScale(realScale)
                        #from steam_example.py panda3d samples
                        self.p = ParticleEffect()
                        self.loadParticleConfig('ptf/fountain.ptf')
                        self.spoutCounter +=1


                self.landCounter += 1
    
    #steam_example.py panda3d samples
    def loadParticleConfig(self, filename):
        ...
        # toplevel
        self.p.start(self.spouts[self.spoutCounter])
