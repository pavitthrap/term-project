...
class MyApp(ShowBase):
    def __init__(self):
        ...
        taskMgr.doMethodLater(self.timer, self.changeGlow, 'changeGlow')
        ###################
        self.inSim = False
        self.drawHomeScreen()
    ...
    def changeGlow (self,task):
        if self.keyMap['glow']>0:
            if self.glowSize<3.9:
                self.glowSize +=.1
                print(self.glowSize)
        elif self.keyMap['unglow']>0:
            if self.glowSize>0:
                self.glowSize-=.1
    ...
    def expandGalaxies(self,task): #!!!!!! make exponential?
        ...
        for gal in range(1, self.counter):
            x,y,z = self.galaxyDistances[gal]
    ...
    def loadGalaxy(self,name,position):
        ...
        self.galaxyDistances[self.counter] = xyzPos
