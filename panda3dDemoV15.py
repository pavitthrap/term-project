...
class MyApp(ShowBase):
    ...
    def closeGalaxyMenu(self):
        ...
        if self.newMiddleGal != 'Current': #then we need to switch gal
            num = int(self.newMiddleGal[len(self.newMiddleGal)-1]) #!!!!!!!!!!!!! newMiddleGal??
    ...
    def changePlanetColor(self,num):
        ...
        for land in self.land:
            print(self.planetTextures[mod][1])
            self.land[land].setColor(*self.planetTextures[mod][1])
    ...
    def drawSlider(self):
        ...
        startYearText='0 years'
        self.textObject= OnscreenText(text=startYearText, pos=(-.7,.70), scale=.07, fg=(1,.5,.5,1), align=TextNode.ACenter, mayChange=1)
        pText='13.8 billion years'
        self.textObject= OnscreenText(text=pText, pos=(0.7,.8), scale=.07, fg=(1,.5,.5,1), align=TextNode.ACenter, mayChange=1)
    ...
    def loadPlanetTextures(self):
        ...
        tex = True
        rgbPlanets = [(209,49,27),(31,190,41), (182, 218, 240), (81,167,89)]
    ...
    def loadTerrain(self):
        ...
        for xPos in range(-(3*self.blenderPlaneDim),3*self.blenderPlaneDim+1,self.blenderPlaneDim):
            for yPos in range (-self.blenderPlaneDim,self.blenderPlaneDim*3+1,self.blenderPlaneDim):
                ...
                print(self.planetTextures[self.textureCounter][1])
                land.setColor(*self.planetTextures[self.textureCounter][1])
                land.setPos(0,0,0)
    ...
    ############## Unused Code ##################
    def loadBackground0(self):
        #lighting documentation 
        #https://www.panda3d.org/manual/index.php/Lighting#Ambient_Lights
        
        #point light
        plight1= PointLight('plight')
        plight1.setColor((1.5,1.5,1.5,1)) #change brightness
        plight1NodePath=render.attachNewNode(plight1)
        plight1NodePath.setPos(0,0,0)  
        render.setLight(plight1NodePath)
        #plight1.setAttenuation((.2, 0, 1))
        ...
        plight2= PointLight('plight')
        plight2.setColor((2,2,2,2))
        plight2NodePath=render.attachNewNode(plight2)
        plight2NodePath.setPos(0,0,20)
        self.landRoot.setLight(plight2NodePath) #change to land

        plight3= PointLight('plight')
        plight3.setColor((1.5,1.5,1.5,1.5))
        plight3NodePath=render.attachNewNode(plight3)
        plight3NodePath.setPos(0,50,20)
        self.landRoot.setLight(plight3NodePath) #change to land

        
        #spotlight #adapted from shadows sample (Panda3D)
        self.light = Spotlight("Spot")
        lightNodePath=render.attachNewNode(self.light)
        lightNodePath.setPos(0,0,20)
        #self.landRoot.setLight(lightNodePath) #change to land


        #ambient light
        ambientL=AmbientLight('ambientL')
        ambientL.setColor(VBase4(0,0,0,1))
        alnp = render.attachNewNode(ambientL)
        render.setLight(alnp)
